...
5 'OMMON EX$,PROG$
10 'ZOUNDS - Audio PROGRAMS - 15 OCT 86 rev. 01 DEC 2009
...
50 CLS:KEY OFF
60 COLOR 7,0,0
70 DIM W$(12,2)     'frequency chart variables
80 DIM N$(12)       'names of notes e.g. A,A# etc.
90 DIM C$(9)        'names of chords
100 DIM AB$(255,2)   'keyboard variables
110 DIM X(9)         'fret locations for guitar tuner graphics
120 DIM Q$(8)        'menu lines for guitar tuner
130 DIM A$(21)       'keyboard variables
140 DIM B$(21)       'keyboard variables
150 D$=" Hz."
160 Q$="ß"      'symbol for flat - chr$(167)
170 U$="###,###.###"
180 UL$=STRING$(80,205)
190 LU$=STRING$(80,"ﬂ")
200 E$=STRING$(80,32)
210 GOSUB 4660               'load variables from data
220 LOCATE ,,0  'cursor off
230 '
240 '.....mathematical formulae
250 I=5.000000E-01        'Increment for halftone
260 LF=4.342944E-01 'Log conversion factor to base 10 = 1/log(10)
270 Z=LOG(I)*LF 'Octave factor (Z= -.3010301)
280 N=12        '12 notes (halftones) per octave
290 U=10^(Z/N)  'Halftone increment multiplier (U=.9438742)
300 A=440       'Standard Pitch in Hz. for middle A
310 X=A*U^9     'Middle C (Start of Octave 4) 9th note below middle A (261.6254)
320 Y=A/U^3     'High C   (Start of Octave 5) 3rd note above middle A (523.2512)
330 SS=3.352800E+05  'Speed of sound in mm/sec. (1100 ft./sec.,750 m.p.h.)
340 '
350 COLOR 15,2
...
400 PRINT UL$;
410 PRINT " NOTE: In the following programs the symbol ß denotes flat, e.g. ";
420 PRINT "Dß,Eß,Gß,Aß,Bß"
...
860 INPUT L:IF Z$="1"THEN L=L/2.539999
870 CLS
880 W=22
...
930 PRINT " Fret #";Z$;Z;
940 PRINT USING "≥###.##≥####.#∫###.##≥####.#≥";S,S*2.539999,L-S,(L-S)*2.539999;
950 PRINT SPC(10);"≥";
...
1000 LOCATE 2,49:PRINT "Æƒø"
1010 LOCATE 14,41:PRINT "Æƒoctave"
...
1050 LOCATE 15,53:PRINT USING "of ###.## in (####.# cm)";L,L*2.539999;
1060 LOCATE 24,49:PRINT "ÆƒŸ";
1070 GOTO 4620
1080 '
1090 '.....analyze note
1100 CLS
...
1210 IF N$="@#"THEN N$="G#"
1220 FOR P=1 TO 12:IF N$=LEFT$(N$(P),2) THEN W=Y*U^(13-P):GOTO 1610 'print table
1230 NEXT P:BEEP:PRINT " There is no such note as ";N$:GOTO 4620
1240 'NEXT Z
1250 GOTO 4620     'end
1260 '
1270 '.....analyze frequency
1280 CLS
...
1440 V=INT(YY/ZZ):W=F/2^V:T=X*SQR(U)    'W=HALFTONE, T=QUARTERTONE
...
1780 PRINT UL$;
1790 LOCATE 1,41:PRINT "The symbol ß denotes flat";
1800 LOCATE 24,1
1810 GOTO 4620      'end
1820 '
1830 IF Z<1.000000E-03 THEN Z=0:RETURN
1840 Z=INT(1000*Z+5.000000E-01)/1000:RETURN
1850 R$="m":Z=SS/A:IF Z<1.000000E-03 THEN 1890
...
1970 COLOR 7,0
1980 PRINT " The symbol ";:COLOR 15:PRINT "ß";:COLOR 7:PRINT " signifies flat.";
...
2090 PRINT " Note ≥";TAB(40);;"ƒƒƒƒƒƒ"
2100 PRINT "      ≥";
2110 FOR Z=-1 TO 10                     'print column headings (octaves)
2120  COLOR 7,0:IF Z=4 THEN COLOR 15,0
2130  PRINT USING "######";Z;
2140 NEXT Z
2150 PRINT UL$;
2160 LOCATE CSRLIN-1,7:PRINT "ÿ"
2170 FOR Z=1 TO 12                               'print rows
2180  PRINT " ";W$(Z,1);TAB(7);;"≥";              'key
2190   FOR P=-1 TO 10                            'columns
2200   Q=VAL(W$(Z,2))*2^P
2210   IF Q>2.748999E+01 AND Q<4.186100E+03 THEN COLOR 15,1 ELSE COLOR 7,0
2220   IF Q<100 THEN V$="####.#"ELSE V$="######"
2230   PRINT USING V$;Q;
2240 '  PLAY"mbo3t255l64ml"
2250 '  PLAY LEFT$(W$(Z,1),2)
2260  NEXT P
2270  PRINT ""
2280 NEXT Z
2290 PRINT "      ≥";
...
2320 COLOR 7,0
2330 PRINT UL$;
2340 LOCATE CSRLIN-1,7:PRINT "œ"
2350 GOTO 4620
2360 '
2370 '.....tune guitar
2380 CLS
...
2460 FOR Z=1 TO 9
2470  FOR Y=7 TO 17
2480   LOCATE Y,X(Z):PRINT "€";                'print frets
2490  NEXT Y
2500 NEXT Z
...
2860 A=0
2870 PLAY"o2"
2880 PRINT " Press letter in < > for desired key.....( ß denotes flat ):"
2890 PRINT UL$;
...
3060 CH$=CH$(X)
3070 PRINT " The notes in ";CH$;;" are.....( ß signifies flat ):"
3080 PRINT UL$;
3090 IF X=1 THEN A=1:B=5:C=8:D=1 :E=0:P$="ceg>c"     'maj
3100 IF X=2 THEN A=1:B=5:C=8:D=10:E=1:P$="cega>c"    '6
3110 IF X=3 THEN A=1:B=5:C=8:D=11:E=1:P$="cegb->c"   '7
3120 IF Y=13 THEN Z$=STRING$(3,"_")ELSE Z$=STRING$(3," ")
3130 IF X=4 THEN A=1:B=5:C=8:D=12:E=1:P$="cegb>c"    'maj 7
3140 IF X=5 THEN A=1:B=4:C=7:D=10:E=1:P$="ce-g-a>c"  'dim
3150 IF X=6 THEN A=1:B=5:C=9:D=1 :E=0:P$="ceg+>c"    'aug
3160 IF X=7 THEN A=1:B=4:C=8:D=1 :E=0:P$="ce-g>c"    'min
3170 IF X=8 THEN A=1:B=4:C=8:D=10:E=1:P$="ce-ga>c"   'min 6
3180 IF X=9 THEN A=1:B=4:C=8:D=11:E=1:P$="ce-gb->c"  'min 7
...
3420 RO=5    'row
3430 CO=16   'column
3440 LOCATE RO+0,CO:PRINT"≥  ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€";
3450 LOCATE RO+1,CO:PRINT"≥  ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€";
3460 LOCATE RO+2,CO:PRINT"≥  ﬁ2› ﬁ3›  ≥  ﬁ5› ﬁ6› ﬁ7›  ≥  ﬁ9› ﬁ0›  ≥  ﬁ=› ﬁ€";
3470 LOCATE RO+3,CO:PRINT"≥  ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€›  ≥  ﬁ€› ﬁ€";
3480 LOCATE RO+4,CO:PRINT"≥   ≥   ≥   ≥   ≥   ≥   ≥   ≥   ≥   ≥   ≥   ≥   ≥";
3490 LOCATE RO+5,CO:PRINT"≥ q ≥ w ≥ e ≥ r ≥ t ≥ y ≥ u ≥ i ≥ o ≥ p ≥ [ ≥ ] ≥";
3500 LOCATE RO+6,CO:PRINT"¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡ƒƒƒ¡";
...
3690 '
3700 '.....continuous range of tones
3710 CLS
...
3970 '
3980 SOUND Y,25    'sustain tone for short time
3990 LOCATE 5,1
...
4430 Z$=INKEY$
4440 IF Z$<>""THEN 4640
4450 SOUND F1,9.100001
4460 SOUND F2,9.100001
4470 GOTO 4430
4480 '
4490 '.....warning
4500 COLOR 12,0:T=6
...
4650 '
4660 '.....data
4670 DATA "C ","C#/Dß","D ","D#/Eß","E ","F "
4680 DATA "F#/Gß","G ","G#/Aß","A ","A#/Bß","B "
4690  FOR Z=1 TO 12:READ N$(Z):NEXT Z               'names of keys
4700 '
4710 DATA maj,6,7,maj 7,dim,aug,min,min 6,min 7
4720  FOR Z=1 TO 9:READ C$(Z):NEXT Z                'names of chords
4730 '
4740 DATA 10,11,23,34,44,53,61,68,74
4750  FOR Z=1 TO 9:READ X(Z):NEXT Z                 'location of frets
4760 '
4770 DATA (1)  E,(2)  B,(3)  G,(4)  D,(5)  A,(6)  E,(7)  Stop sound,(8)  EXIT
4780  FOR Z=1 TO 8:READ Q$(Z):NEXT Z                'guitar strings menu
4790 '
...
4820 DATA " ",p64
4830  FOR Z=1 TO 21:READ A$(Z),B$(Z):NEXT Z         'keyboard notes
4840 RETURN
4850 '
4860 'PRT
